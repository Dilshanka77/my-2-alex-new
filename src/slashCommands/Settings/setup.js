...
//        id: cl/ient.user.id,
       ...
       //     const voiceChannel = await interaction.guild.channels.create(`${client.user.username} Music`, {
        ...
        //    });
            ...
            const but1 = new MessageButton().setCustomId(`${interaction.guildId}pause`).setLabel(`pause & resumed`).setEmoji(`⏯`).setStyle('SECONDARY').setDisabled(true)
            const but2 = new MessageButton().setCustomId(`${interaction.guildId}previous`).setLabel(`previous`).setEmoji(`⏮️`).setStyle('SECONDARY').setDisabled(true)
            const but3 = new MessageButton().setCustomId(`${interaction.guildId}skip`).setLabel(`skip`).setEmoji(`⏭️`).setStyle('SECONDARY').setDisabled(true)
            const but4 = new MessageButton().setCustomId(`${interaction.guildId}voldown`).setLabel(`voldown`).setEmoji(`🔉`).setStyle('SECONDARY').setDisabled(true)
            const but5 = new MessageButton().setCustomId(`${interaction.guildId}volup`).setLabel(`volup`).setEmoji(`🔊`).setStyle('SECONDARY').setDisabled(true)
            const but6 = new MessageButton().setCustomId(`${interaction.guildId}stop`).setLabel(`stop`).setEmoji(`⏹`).setStyle('SECONDARY').setDisabled(true)
            const but7 = new MessageButton().setCustomId(`${interaction.guildId}autoplay`).setLabel(`autoplay`).setEmoji(`🔀`).setStyle('SECONDARY').setDisabled(true)
           // const but8 = new MessageButton().setCustomId(`${interaction.guildId}loop`).setEmoji(`🔁`).setStyle('SECONDARY').setDisabled(disabled)
