const { MessageEmbed, MessageActionRow, MessageSelectMenu, CommandInteraction, Client, MessageButton  } = require('discord.js');
...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    const row4 = new MessageActionRow().addComponents(
      new MessageButton().setLabel('invite').setStyle('LINK').setURL(invite).setEmoji(),
      new MessageButton().setLabel('support').setStyle('LINK').setURL(support).setEmoji(),
      new MessageButton().setLabel('vote').setStyle('LINK').setURL(vote).setEmoji(),
    ...
    const m = await interaction.editReply({ embeds: [embed], components: [row,row4], ephemeral: true});
     //await interaction.reply({content: `There is no player for this guild.`,ephemeral: true, })
    ...
    collector.on('collect', (interaction) => {
      ...
      if (options === 'music') {
        _commands = client.commands
          ...
          .map((x) => `\`${x.name}\` **:-** _${x.description}_`);
