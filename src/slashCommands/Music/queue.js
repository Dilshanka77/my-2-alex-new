const { Client, CommandInteraction, MessageEmbed, MessageButton, MessageActionRow, Message } = require('discord.js');
    ...
    if (player.queue.length === '0' || !player.queue.length) {
      ...
      if (player.queue.length < 11) {
        const embed2 = new MessageEmbed()
          ...
          .setFooter({
            text: `Requested By :- ${interaction.user.tag} | Page ${page + 1}/${pages.length}`,
          ...
          .setTitle(`${interaction.guild.name} Queue \`[${player.queue.length}]\``);
        await interaction.editReply({
            embeds: [embed2],
          })
          .catch(() => {});
