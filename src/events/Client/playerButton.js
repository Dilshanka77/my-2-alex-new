...
module.exports = {
    ...
    run: async (client, interaction, data) => {
        ...
        } else if (interaction.customId === `${interaction.guildId}stop`) {
            if (!player.current) {
                 await buttonReply(interaction, `not playing Music plss play mysic and use the command`, color);
                }
              ...
              await buttonReply(interaction, `${emojistop} Stopped the music`, color);
            if (message) await message.edit({
                embeds: [nowplaying]
            }).catch(() => { });
        }else if (interaction.customId === `${interaction.guildId}autoplay`){ 
