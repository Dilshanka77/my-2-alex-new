...
module.exports = {
 ...
 // let Name = args.slice(0).join(" ");
    ...
    if (!Name) return message.reply({
      embeds: [
        new MessageEmbed()
          ...
          .setDescription(`Playlist not found. Please enter the playlist name and playlist song number`),
      ],
    });
    //const Name = args[0];
    ...
    if (!Options) return message.reply({
      embeds: [
        new MessageEmbed()
          .setColor(client.embedColor)
          .setDescription(`Playlist not found. Please enter the playlist name and playlist song number`),
          ...
          new MessageEmbed()
            ...
            .setDescription(
              `Your provided track number is out of Range (\`0\` - ${
                tracks.length - 1
              })\nSee all your Tracks: \`${prefix}info ${Name}\` showdetails ${Name}`,
            ),
        ],
      });
    }
