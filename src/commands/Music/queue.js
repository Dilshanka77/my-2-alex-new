    ...
    if (player.queue.length === '0' || !player.queue.length) {
      ...
      if (player.queue.length < 11) {
        const embed = new MessageEmbed()
          ...
          .setFooter({
            text: `Requested By :- ${message.author.tag} | Page ${page + 1}/${pages.length}`,
          ...
          .setTitle(`${message.guild.name} Queue \`[${player.queue.length}]\``);
        ...
        const but32 = new MessageButton()
          ...
          const row1 = new MessageActionRow().addComponents([but21, but32, but13]);
          await msg.edit({
            components: [row1],
          });
        });
